...
/** Sem alunoId: inscrição do trainer. Com alunoId: inscrição do aluno. */
export default function PushToggle({ alunoId }: { alunoId?: string }) {
    ...
    try {
      if (!ativo) {
        ...
        const { error } = await supabase.from("push_subscriptions").upsert(
          alunoId
            ? { ...base, aluno_id: alunoId, trainer_id: null }
            : { ...base, trainer_id: user!.id, aluno_id: null },
          { onConflict: "endpoint" }
        );
        if (error) {
